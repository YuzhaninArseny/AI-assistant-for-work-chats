...
from bot_app.commands.groups_kb import ACTION_PAGE, create_kb, ACTION_EMPTY
...
async def draft_cmd_start(message: Message, aiohttp_session: ClientSession):
    groups = await get_user_groups(aiohttp_session, message.from_user.id)
    markup = create_kb(groups, ACTION_DRAFT, DraftCBDataFactory)
...
async def group_search(callback: CallbackQuery, callback_data: DraftCBDataFactory, aiohttp_session: ClientSession):
    group_id = callback_data.payload
    await callback.answer()
    await callback.message.edit_text(f"Генерируем ответ для {group_id}. Результат пришлём новым сообщением")
...
async def draft_page_cb(callback: CallbackQuery, callback_data: DraftCBDataFactory, aiohttp_session: ClientSession):
    groups = await get_user_groups(aiohttp_session, callback.message.from_user.id)
    markup = create_kb(groups, ACTION_DRAFT, DraftCBDataFactory, current_page=callback_data.payload)
